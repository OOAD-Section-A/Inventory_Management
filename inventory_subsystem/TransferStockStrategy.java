...
public class TransferStockStrategy {
    ...
    public void transfer(String productId, String fromLocation,
        ...
        source.recomputeTotalQuantity();

        if (source.getAvailableQuantity() < quantity) {
            exceptions.onInsufficientStock(productId, quantity, source.getAvailableQuantity());
            return;
        }
        ...
        while (iterator.hasNext() && remaining > 0) {
            ...
            if (repository instanceof InventoryRepository repo) {
                repo.recordTransaction(
                    new StockTransaction(productId, batchIdForTx, fromLocation, -moveQty, "TRANSFER_OUT", "TRANSFER", "AUTO")
                );
                repo.recordTransaction(
                    new StockTransaction(productId, batchIdForTx, toLocation, moveQty, "TRANSFER_IN", "TRANSFER", "AUTO")
                );
            }
        }
